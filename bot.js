...
client.user.setGame(`BOX SHOP || BroadCast`,"http://twitch.tv/Death")
        ...
        if(!message.channel.guild) return;
            ...
            if(command == "bc") {
                if(!message.member.hasPermission("ADMINISTRATOR")) {
                    return message.channel.send("**ما معك صلاحية ادمن**");
                }
                    if(!args) {
                        return message.reply("**اكتب في بالبرودكاست ما تخلية فاضي**");
                    }
                        message.channel.send(`**بدك ترسل الكلام\nمحتوى البرودكاست: \`${args}\`**`).then(m => {
                            m.react("✅")
                            .then(() => m.react("❌"));
                                ...
                                m.delete();
                                    message.channel.send(`:ballot_box_with_check: | ياالحبيب الرسالة راحت للناس ${message.guild.memberCount} Members`).then(msg => msg.delete(5000));
                                        message.guild.members.forEach(member => {
                                            ...
                                            addField("الراسل", message.author.username)
            ...
            if(command == "bco") {
                if(!message.member.hasPermission("ADMINISTRATOR")) {
                    return message.channel.send("**ما معك ادمن`**");
                }
                    if(!args) {
                        return message.reply("**اكتب شي بالبرودكاست لا تخلية فاضي**");
                    }
                        message.channel.send(`**بدك ترسل الرسالة\nمحتوى البرودكاست: \`${args}\`**`).then(m => {
                            m.react("✅")
                            .then(() => m.react("❌"));
...
client.on("message", async message => {
    if(message.content.startsWith(prefix + "help")) {
        let help = new Discord.RichEmbed()
            ...
            دعوة البوت لسيرفرك : ${prefix}invite `معطله`
